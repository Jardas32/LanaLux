...
function priceQuantity() {
  let totalPrice = storage.reduce((pre, item) => {
        return pre + item.priceNumber * item.quantity;
  },0)
   document.querySelector('.priceTotal').textContent = `${totalPrice}`;
}
...
const renderCart = () => {
  ...
  if (storage) {
      ...
      const {cartId, ptitle, priceNumber, quantity = 1} = el;
      ...
      <div class="box">
      <p class="p3"> ${priceNumber * quantity} <img src="img/Far.png" alt=""></p>
      </div>
      <div class="box">
      <div class="calculato counter" data-counter>
      <div data-index="${index}" class="btMinus divcalcul divcalc counter__button counter__button__minus">-</div>
      <div class="divcalcul counter__input"><input class="inputcount" type="text" value="${quantity}"></div>
      <div data-index="${index}" class="btPlus divcalcul divcalc2 counter__button counter__button__plus">+</div> 
      </div>
      </div>
      <div class="box">
      <button class="divx btkk" card-count="${index}">X</button>
      </div>
      </div>`;
...
if(storage.length <= 0) {
  h2korzina.innerHTML = `Ваша Корзина Пуста`;
  document.querySelector('.akorz').style.display = `flex`;
}

cartSide.addEventListener('click', (e) => {

...
cards.forEach((el) => {
  ...
  let priceNumber = parseInt(pcena.replace(/^\g/, ''), 10);
  
  btkk.addEventListener(`click`, () => {

    const card = {cartId, ptitle, priceNumber, btkk, quantity: 1};
    
    const cardStoreg = localStorage.getItem('cart') || "[]";
    const cart = JSON.parse(cardStoreg);

    const existCart = cart.findIndex((item) => item.cartId === cartId);
    if(existCart !== -1) {
...
const pricePopup = document.querySelector('.pricePopup');
      ...
      cartSet.forEach(el => {
        const openPopup = el.childNodes[1];
        const img = el.childNodes[1].attributes.src.textContent;
        const ptitle = el.childNodes[3].textContent;
        const pcena = el.childNodes[7].innerText;

openPopup.addEventListener('click', () => {

       imgPopup.src = img;
       titlePopup.textContent = ptitle;
       pricePopup.textContent = pcena;
...
document.querySelector('.btPopup').addEventListener('click', () => {
